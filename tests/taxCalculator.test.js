...
import { calculateTaxes, calculateNetIncome } from "../src/utils/taxCalculator.js";
...
describe("tax calculation logic", () => {
  it("calculates monthly taxes correctly", () => {
    ...
    expect(taxData.taxes[0]).toEqual(expect.objectContaining({ label: "Єдиний податок", value: 750 }));
    expect(taxData.taxes[1]).toEqual(expect.objectContaining({ label: "Військовий збір", value: 150 }));
  ...
  it("calculates quarterly taxes correctly", () => {
    ...
    expect(taxData.taxes).toHaveLength(3);
    expect(taxData.taxes[0]).toEqual(expect.objectContaining({ label: "Єдиний податок", value: 7500 }));
    expect(taxData.taxes[1]).toEqual(expect.objectContaining({ label: "Військовий збір", value: 1500 }));
